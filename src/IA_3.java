...
public class IA_3 extends GameEntity {
	 ...
	 * @param num
	 */
	public IA_3(TronGame game, int dir, Color couleur, int num, int x, int y){
		xPos=x;
		yPos=y;
		xPos_init=xPos;
		yPos_init=yPos;
	...
	public void run(){
		...
		while(true)
		{
			while(this.isVisible()){
				...
				decideDirection(tron.getEntity(numRntityToFollow).getX(),tron.getEntity(numRntityToFollow).getY());
				
				//on ne dessine qu'une entitité à la fois sinon les couleurs se mélangent
	...
	/**
	 * choisit l'entité à suivre en fonction de celle(s) qui 
	 * dont encore en vie
	 */
