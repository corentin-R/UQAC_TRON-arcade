...
public class TronGame extends JPanel implements KeyListener, ActionListener {
	...
	public void initPartie() {
		...
		timer = new Timer(2, this);// génere des impulsions tous les x ms, créer
									// un framerate

		offScreenGraphics.clearRect(0, 0, 1016, 736);
		offScreenGraphics.setColor(Color.white);
		offScreenGraphics.fillRect(10, 10 + 60, getWidth() - 20,
				getHeight() - 60 - 20);
		...
		entities[0] = new GameEntity(this, direction.nextInt(4) * 90,
				Color.BLUE, 0, getWidth() / 6, getHeight() / 2);
		entities[1] = new IA_1(this, direction.nextInt(4) * 90, Color.RED, 1,
				5 * getWidth() / 6, getHeight() / 2);
		entities[2] = new IA_2(this, direction.nextInt(4) * 90, Color.GREEN, 2,
				getWidth() / 2, getHeight() / 4);
		entities[3] = new IA_3(this, direction.nextInt(4) * 90, Color.YELLOW,
				3, getWidth() / 2, 3 * getHeight() / 4);
	...
	public void initRound() {
		...
		for (GameEntity it : entities) {
			// System.out.println(it.getState()==Thread.State.WAITING);
			if (!it.isAlive()) {// on réveille les entités mortes
				it.setVisible();
				it.notify();
			}
			it.reset();
	...
	public void afficherVictoire(Graphics g) {
		whoWon();// obligé de vérifier pour mettre partieOver à true avnt de le
					// vérifier
	...
	/**
	 * vérifie si chacun des GameEntity va mourir et vérifie combien d'entités
	 * sont vivantes
	 * 
	...
	/**
	 * décide si c'est la fin d'un round ou de la partie et qui a gagné
	 * 
	 ...
	 * 
	 * @param x
	 *            , position horizontale du pixel à tester
	 * @param y
	 *            , position verticale du pixel à tester
	 */
